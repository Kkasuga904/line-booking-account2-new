// Smoke Test Script for Account 2 (ES Module version)
// Usage: node smoke-test-account2.mjs [DEPLOYMENT_URL]
...
const DEPLOY_URL = process.argv[2] || 'https://line-booking-account2-new.vercel.app';

console.log(`🔍 Starting smoke test for Account 2: ${DEPLOY_URL}`);
console.log('================================\n');
...
async function runTests() {
  ...
  await testEndpoint('/admin.html', 200, 'Admin page');
  await testEndpoint('/admin-calendar.html', 200, 'Calendar page');
  await testEndpoint('/liff-calendar.html', 200, 'LIFF page');
  ...
  await testEndpoint('/api/capacity', 200, 'Capacity API');
  
  // 4. Webhook Tests
  console.log(`\n${colors.yellow}4. Webhook Tests${colors.reset}`);
  
  // Test webhook with empty events
  await testPostEndpoint('/api/webhook', { events: [] }, 200, 'Webhook with empty events');
  ...
  await testPostEndpoint('/api/webhook', followEvent, 200, 'Webhook with follow event');
  ...
  await testPostEndpoint('/api/webhook', messageEvent, 200, 'Webhook with message event');
